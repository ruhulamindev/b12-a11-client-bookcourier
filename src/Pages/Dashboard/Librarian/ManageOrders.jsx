...
const ManageOrders = () => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const res = await axios.get(
        `https://b12-a11-server-bookcourier.vercel.app//orders/librarian?email=${user.email}`
      );
      return res.data;
    },
  });
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    await axios.patch(`https://b12-a11-server-bookcourier.vercel.app//orders/status/${orderId}`, {
      status: newStatus,
    });
  ...
  const handleCancel = async (orderId) => {
    ...
    await axios.patch(`https://b12-a11-server-bookcourier.vercel.app//orders/cancel/${orderId}`);
    refetch(); // auto page update no refash
  };
